          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div className="bg-slate-50 rounded-lg p-4 border border-slate-200 flex flex-col justify-between">
              ...
              <div className="bg-white p-2.5 rounded border border-slate-200 text-[11px] font-mono text-slate-700">
                <span className="font-bold text-indigo-700 block mb-0.5">Формула / Formula:</span>
                Rate = (Σ General - Base) / Total People
              </div>
            </div>
            ...
            <div className="bg-slate-50 rounded-lg p-4 border border-slate-200 flex flex-col justify-between">
              ...
              <div className="bg-white p-2.5 rounded border border-slate-200 text-[11px] font-mono text-slate-700">
                <span className="font-bold text-emerald-700 block mb-0.5">Формула / Formula:</span>
                Rate = Σ Maint / Total People
              </div>
            </div>
            ...
            <div className="bg-slate-50 rounded-lg p-4 border border-slate-200 flex flex-col justify-between">
              <div>
                <div className="flex items-center gap-2 text-amber-600 font-bold text-xs mb-2">
                  ...
                  <span>{isBg ? 'Гаражи и електроенергия' : 'Garages & Power'}</span>
                </div>
                <p className="text-xs text-slate-600 mb-3">
                  {isBg 
              ...
              <div className="bg-white p-2.5 rounded border border-slate-200 text-[11px] font-mono text-slate-700">
                <span className="font-bold text-amber-700 block mb-0.5">Формула / Formula:</span>
                Rate = Σ Garage / Total Garage Cells
              </div>
            </div>
            ...
            <div className="bg-slate-50 rounded-lg p-4 border border-slate-200 flex flex-col justify-between">
              <div>
                <div className="flex items-center gap-2 text-sky-600 font-bold text-xs mb-2">
                  ...
                  <span>{isBg ? 'Фонд Ремонт (% Ид. части)' : 'Repair Fund (% Parts)'}</span>
                </div>
                <p className="text-xs text-slate-600 mb-3">
                  {isBg 
                    ? 'Разпределя се строго пропорционално според процента идеални части на имота по нотариален акт.'
                    : 'Distributed strictly proportional to the apartment\'s percentage of common building areas.'}
                </p>
              </div>
              <div className="bg-white p-2.5 rounded border border-slate-200 text-[11px] font-mono text-slate-700">
                <span className="font-bold text-sky-700 block mb-0.5">Формула / Formula:</span>
                Due = Target × (% Ideal Parts / 100)
              </div>
            </div>
          </div>
          ...
          <div className="bg-indigo-900 text-white rounded-lg p-4 text-xs font-mono flex flex-col md:flex-row items-center justify-between gap-4">
            <div>
              ...
              <p className="text-sm font-bold text-indigo-100">
                Total Due = (People × (Gen + Maint)) + (Garages × GarageRate) + (RepairTarget × %Parts/100) + ElevatorBase + Arrears
              </p>
            </div>
            <div className="bg-white/10 px-3 py-2 rounded text-[11px] text-indigo-200 border border-white/10 text-center">
              {isBg ? 'Автоматично се изчислява в реално време' : 'Automatically computed in real-time'}
            </div>
          </div>
      ...
      {(activeSection === 'all' || activeSection === 'bills') && (
        <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-6 space-y-6">
          <div className="flex items-center gap-3 border-b border-slate-100 pb-4">
            ...
            <div>
              <h2 className="text-lg font-bold text-slate-800">
                {isBg ? '2. Постоянни vs. Променливи сметки и обновяване на такси' : '2. Fixed vs. Variable Monthly Bills & Tax Updates'}
              </h2>
              <p className="text-xs text-slate-500">
                {isBg 
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="border border-indigo-200 bg-indigo-50/30 rounded-xl p-5 space-y-3">
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-sm font-bold text-slate-800">
                  {isBg ? 'Постоянни разходи с авто-прехвърляне' : 'Fixed Auto-Carried Expenses'}
                </h3>
              </div>
            ...
            <div className="space-y-1">
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs font-bold uppercase tracking-wider text-indigo-300">
                  {isBg ? 'Бутон "Изчисли и обнови таксите"' : 'Calculate & Update Taxes Button'}
                </span>
              </div>
